...
function parseQuery(query) {
  ...
  const selectRegex = /^SELECT\s(.*?)\sFROM\s(.*?)(?:\s(INNER|LEFT|RIGHT)\sJOIN\s(.*?)\sON\s(.*?))?\s?(WHERE\s(.*?))?$/i;
  ...
  if (matches) {
    const fields = matches[1].split(',').map(field => field.trim());
    const table = matches[2].trim();
    const joinType = matches[3] ? matches[3].trim() : null;
    const joinTable = matches[4] ? matches[4].trim() : null;
    const joinCondition = matches[5] ? parseJoinCondition(matches[5].trim()) : null;
    const whereClause = matches[7] ? matches[7].trim() : null;
    const whereClauses = whereClause ? parseWhereClause(whereClause) : [];

    return { fields, table, whereClauses,  joinCondition, joinTable,joinType };
  } else {
    throw new Error('Invalid query format');
  }
}
